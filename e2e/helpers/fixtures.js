...
const {session}=require('./identity');
const test=base.extend({
  ...
  signedIn:async({page,context,baseURL,account},use)=>{
    await context.addCookies([{name:'blak_session',value:await session(account,'E2E Tester'),url:baseURL,httpOnly:true,sameSite:'Lax'}]);
    await use(page);
