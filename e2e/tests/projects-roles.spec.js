...
test('native Projects roles constrain existing keys, sessions and managed authority', async ({ page, context }) => {
  ...
  async function response(token, method, path, data) {
    return fetch(endpoint + path, { method, headers: { 'x-api-key': token, origin, 'content-type': 'application/json' }, ...(data === undefined ? {} : { body: JSON.stringify(data) }) });
  }
  ...
  async function waitRole(role) {
    await expect.poll(async () => {
      const user = await api(operator['api-key'], 'GET', '/api/auth/admin/get-user?id=' + native.id);
      const organization = await api(operator['api-key'], 'GET', organizationPath);
  ...
  try {
    ...
    if (project) await api(operator['api-key'], 'DELETE', '/api/project/' + project.id);
    if (native) await api(operator['api-key'], 'POST', '/api/auth/admin/remove-user', { userId: native.id });
  }
});
