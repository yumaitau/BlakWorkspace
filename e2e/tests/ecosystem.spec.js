...
test('Drive SSO renders files and shared brand themes', async ({ page, request }) => {
  await page.goto('https://drive.workspace.example.com');
  await authentikLogin(page);
  await page.waitForURL(url => url.hostname === 'drive.workspace.example.com' && !url.pathname.includes('oidc-callback'));
  await expect(page.getByRole('button', { name: /New/i }).first()).toBeVisible({ timeout: 45000 });
  const theme = await (await request.get('https://drive.workspace.example.com/themes/blak/theme.json')).json();
...
test('Docs discovery and Hermes custom theme are available', async ({ request }) => {
  const docs = await request.get('https://docs.workspace.example.com/hosting/discovery');
  expect(docs.ok()).toBeTruthy();
  expect(await docs.text()).toContain('wopi-discovery');
  const css = await request.get('https://hermes.workspace.example.com/static/custom.css');
