...
function blakTheme(logo = 'themes/blak/assets/logo.svg') {
  ...
  return { common:{name:'Blak Drive',slogan,logo},clients:{web:{defaults:{logo,favicon:logo,slogan,background:'/_blak/login-background.png'},themes:['dark','light'].map(mode=>({isDark:mode==='dark',label:`Blak ${mode === 'dark' ? 'Dark' : 'Light'}`,designTokens:{roles:driveRoles(mode)}}))}}};
}
