...
REDIRECTS = [RedirectURI(matching_mode=RedirectURIMatchingMode.STRICT, url="http://portal.workspace.example.com/callback")]
...
provider, created = OAuth2Provider.objects.update_or_create(name="Blak Portal", defaults={"authorization_flow": auth_flow, "invalidation_flow": inval_flow, "client_type": "confidential", "client_id": "blak-portal", "redirect_uris": REDIRECTS, "signing_key": key, "sub_mode": "user_username", "include_claims_in_id_token": True, "issuer_mode": "per_provider"})
new_secret = secrets.token_urlsafe(32)
provider.client_secret = new_secret
provider.save()
provider.property_mappings.set(mappings)
app, _ = Application.objects.update_or_create(slug="blak-portal", defaults={"name": "Blak Portal", "provider": provider, "open_in_new_tab": True})
