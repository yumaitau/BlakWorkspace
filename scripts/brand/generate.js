...
const shellApps = [{ id:'portal', name:'Workspace', url:'https://portal.workspace.example.com/', backend:'Blak Workspace by Yuma IT' }, ...APPS.filter(a=>a.status==='live' && !a.hidden && a.url).map(a=>({id:a.id,name:a.name,url:new URL(a.url,'https://portal.workspace.example.com').href,backend:a.backend}))];
