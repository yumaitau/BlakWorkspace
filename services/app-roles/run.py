...
def reconcile(session):
    ...
    if not config or not set(config) <= {'vault', 'hermes', 'projects', 'knowledge', 'crm', 'forms', 'chat', 'drive'}:
        raise ValueError('Unknown or empty native role configuration')
    failures = []
    if config.get('drive'):
        try:
            settings = config['drive']
            result = drive.reconcile(API(settings['base'], settings['token']), directory)
            print('Drive roles reconciled ' + json.dumps(result, sort_keys=True), flush=True)
        except Exception as error:
            failures.append('Drive:' + type(error).__name__)
    if config.get('chat'):
        try:
